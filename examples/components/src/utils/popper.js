...
;(function (root, factory) {
    ...
    /**
     * 修改 this._options 属性的参数 by wbtian
     */
    Popper.prototype.updateOptions = function(option) {
    ...
    Popper.prototype._getOffsets = function(popper, reference, placement) {
        ...
        if (['right', 'left'].indexOf(placement) !== -1) {  // 如果在水平方向，应当和相关元素垂直居中对齐
            ...
            if (placement === 'top') {  // 如果在上边，则 top 应当为相关元素的 top 减去 popper 的高度 若为负数 说明会被上方挡住，则应设置成0
                popperOffsets.top = (referenceOffsets.top - popperRect.height)<0 ? 0 : (referenceOffsets.top - popperRect.height);
    ...
    Popper.prototype._getBoundaries = function(data, padding, boundariesElement) {
        ...
        if (boundariesElement === 'window') {  // 如果边界元素是窗口
            ...
            let offsetParentFlag = offsetParent.getAttribute("popper-deal-flag");

            boundaries = {
                top: 0 - (offsetParentRect.top - scrollTop),
                //right: root.document.documentElement.clientWidth - (offsetParentRect.left - scrollLeft),
                right: offsetParentRect.width - (offsetParentRect.left - scrollLeft),//修正 prevent-over-id有值时 应该取父元素的长度
                bottom: offsetParentRect.bottom - (offsetParentRect.top - scrollTop),//todo appendToBody必须为true
                //bottom: root.document.documentElement.clientHeight- (offsetParentRect.top - scrollTop),
                left: 0 - (offsetParentRect.left - scrollLeft)
            };
            //todo 处理一些不能如何解决的bug
            switch (offsetParentFlag){
                case "1":
                    boundaries.right = root.document.documentElement.clientWidth - (offsetParentRect.left - scrollLeft);//解决有些用法不当的 造成的问题
                    break;
            }
        } else {
    ...
    function getOffsetParent(element) {
        ...
        //是否有preventOverId属性 用来设置容器的
        if(this && this._options && this._options.preventOverId && document.getElementById(this._options.preventOverId)){
            return document.getElementById(this._options.preventOverId);
        }else {
